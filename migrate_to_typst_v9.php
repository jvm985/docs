...
$targetProject = Project::create([
    ...
    'user_id' => $user->id,
    'description' => 'Final Exam Grade Typst Version - V9',
]);
...
#let answer(height, body) = block(
  ...
  [ #set line(stroke: (dash: "dotted", thickness: 0.5pt, paint: gray.lighten(30%))) #repeat[#line(length: 100%) #v(0.7cm)] #place(top + left)[#body] ]
)
...
function convertToTypst($content) {
    ...
    $content = preg_replace_callback('/\\\\begin\{(?:long)?tblr\}(?:\[.*?\])?\{(.*?)\}(.*?)\\\\end\{(?:long)?tblr\}/s', function($m) {
        $colCount = preg_match_all('/[XlrcQ]/', $m[1], $matches); $cols = $colCount ? $colCount : 2;
    ...
    $content = str_replace('\end{opdracht}', "\n]\n", $content);

...
$targetProject = Project::create([
    ...
    'user_id' => $user->id,
    'description' => 'Perfected Typst Version - V11 (Exact PDF Match)',
]);
...
#let base-box(title: "", color: black, body) = block(
  fill: color.lighten(95%), stroke: (left: 3pt + color), inset: 12pt, width: 100%, breakable: true,
  [ #if title != "" { text(weight: "bold", fill: color, size: 9pt)[#smallcaps(title)] v(4pt) } #body ]
)
...
#let answer(height, body) = block(
  ...
  fill: pattern(size: (100%, 0.635cm))[ #place(bottom)[#line(length: 100%, stroke: (dash: "dotted", thickness: 0.5pt, paint: gray.lighten(30%)))] ],
  [ #place(top + left)[#body] ]
)
...
// --- CUSTOM TABLES ---
#let bronanalyse() = table(
  columns: (auto, 1fr, auto, auto),
  stroke: (x, y) => if x == 0 and y <= 1 { (right: 1pt + black) } else if y == 0 { (bottom: 1pt + black) } else { none },
  align: left + horizon,
  [*Bronanalyse*], table.cell(colspan: 3)[#answer(1cm)[]],
  [Soort bron:], [#answer(1cm)[]], [$square$ Primair], [$square$ Secundair],
  [Datering:], [#answer(1cm)[]], [$square$ Objectief], [$square$ Subjectief],
  [Auteur:], [#answer(1cm)[]], [$square$ Betrouwbaar], [$square$ Onbetrouwbaar],
  [], [], [$square$ Bruikbaar], [$square$ Onbruikbaar],
  [Doel:], table.cell(colspan: 3)[#answer(1.5cm)[]]
)

#let historisch_begrip(title) = table(
  columns: (auto, 1fr), stroke: 1pt + black, align: left + horizon,
  table.header([*Wat*], [*#title*]),
  [*Wie*], [#answer(1cm)[]], [*Waar*], [#answer(1cm)[]], [*Wanneer*], [#answer(1cm)[]],
  [*Aanleiding/Oorzaken/Doelen*], [#answer(1.5cm)[]], [*Inhoud*], [#answer(1cm)[]], [*Gevolgen*], [#answer(1cm)[]]
)

#let historisch_vergelijken(col1, col2) = table(
  columns: (auto, 1fr, auto, 1fr), stroke: 1pt + black, align: left + horizon,
  [*aspect*], [*#col1*], [$= eq.not$], [*#col2*],
  [], [#answer(1cm)[]], [], [#answer(1cm)[]], [], [#answer(1cm)[]], [], [#answer(1cm)[]],
  [], [#answer(1cm)[]], [], [#answer(1cm)[]], [], [#answer(1cm)[]], [], [#answer(1cm)[]],
  [], [#answer(1cm)[]], [], [#answer(1cm)[]]
)

#let historisch_argumenteren(col1, col2) = table(
  columns: (1fr, 1fr), stroke: 1pt + black, align: left + top,
  [*#col1*], [*#col2*], [#answer(4cm)[]], [#answer(4cm)[]]
)

#let concepts(body) = table(columns: (4cm, 1fr), stroke: 0.5pt + gray, fill: (x, y) => if y == 0 { olive.lighten(80%) }, [*Historisch Begrip*], [*Omschrijving*], ..body)
#let aims(body) = table(columns: (1fr), stroke: 0.5pt + gray, fill: olive.lighten(80%), [*Na het lezen van dit hoofdstuk kun je:*], body)
#let tblr(columns: 1, ..args) = table(columns: (1fr,) * columns, stroke: 0.5pt + gray, inset: 8pt, align: left + top, ..args)
...
function convertToTypst($content) {
    ...
    $content = preg_replace('/\\\\HistorischVergelijken\{(.*?)\}\{(.*?)\}/', "\n#historisch_vergelijken([$1], [$2])\n", $content);
    $content = preg_replace('/\\\\HistorischArgumenteren\{(.*?)\}\{(.*?)\}/', "\n#historisch_argumenteren([$1], [$2])\n", $content);

    // 2. Map all known environments
    ...
    // Convert fillwithdottedlines if manually called
    $content = preg_replace('/\\\\fillwithdottedlines\{(.*?)\}/', '#answer($1)[]', $content);
    ...
    // Wrap images in figures and fix paths
    $res = preg_replace_callback('/(?:#figure\(|#box\(width: [^,]+, )?image\("(?:figuren\/)?([^"]+)"\)\)?\s*(<fig:[^>]+>)?/', function($matches) {
        $imgName = $matches[1];
        $label = isset($matches[2]) ? $matches[2] : '';
        $path = '../figuren/' . $imgName;
    ...
    $res = str_replace('width: 00%', 'width: 100%', $res);
...
$main = "#import \"template.typ\": *\n#show: project.with(title: \"05 Geschiedenis\", authors: (\"" . $user->name . "\",))\n"
      . "#align(center)[#text(24pt, weight: \"bold\")[05 Geschiedenis]\\ #v(1cm) #text(14pt)[Full Course V11]]\n"

        ...
        <div class="flex justify-between items-center p-2 bg-gray-100 border-b border-gray-300">
            ...
            <div class="flex gap-4 items-center">
                ...
                <button
                   v-if="['tex', 'typ', 'md', 'rmd', 'r'].includes(ext)"
                   @click="$emit('show-log')"
                   :class="['px-3 py-1 rounded text-sm transition-colors', isShowingLog ? 'bg-orange-500 text-white hover:bg-orange-600' : 'bg-gray-500 text-white hover:bg-gray-600']"
                >
                   {{ isShowingLog ? 'Hide Log' : 'Show Log' }}
                </button>            </div>
        </div>
